...
def linearContrast(img_obj, **kwargs):
    ...
    if kwargs.get('show_imgs'):
        showColoredImgshistogram([img_obj, altered_img])

    return altered_img


def logaritmicContrast(img_obj, show_imgs=False):
    altered_img = copy.copy(img_obj)
...
def variableLinearContrast(img_obj, show_imgs=False):
    altered_img = copy.copy(img_obj)
...
if __name__ == '__main__':
    ...
    #showImages([fro])
    #showColoredImgshistogram([lenna, fro])
    #showGrayImgsHistogram([lenna, fro])

    #showImages([lenna, fro], ['lenna', 'fro'])

    #linearContrast(fro, percentage=-40, show_imgs=True)
    #logaritmicContrast(fro, show_imgs=True)
    #variableLinearContrast(fro, show_imgs=True)
    #quadraticContrast(fro, percentage=60, show_imgs=True)
    #quadraticContrast(gray_fro, percentage=60, show_imgs=True) # => n funciona para imagens escala de cinza, teria que mudar a lógica da função
    #exponencialContrast(fro, show_imgs=True)
    #sqrtContrast(fro, percentage=30, show_imgs=True)

    #equalizeHist(fro, show_imgs_hist=True)
    #showImages([lenna, cannyFilter(lenna, min_val=60, max_val=150)], ['original', 'canny filter'])
